...
module ServiceFeeAdmin =
    ...
    let currentImages = View.Map (fun il -> il |> Seq.filter (fun (_,uid,_) -> uid = SelectedImageUploader.Value)) ImageList.View
                ...
                |Ok l ->
                    ...
                    match userlist with
                        |Ok ul ->
                            ...
                            if SelectedImageUploader.Value = -1 && not <| Seq.isEmpty ImageUserList.Value then SelectedImageUploader.Set <| (Seq.head ImageUserList.Value).Id
        ...
        SiteParts.ServiceTemplate()
            .OwedMemberList(
                OwedUserList.View |> Doc.BindSeqCached(
                    fun u ->
                        SiteParts.ServiceTemplate.OwedMemberItem()
                            .UserID(string u.Id)
                            .Member(u.Name)
                            .Doc()
                )
            ...
            .ServiceList(
                PendingFeeList.View |> Doc.BindSeqCached(
                    fun p ->
                        SiteParts.ServiceTemplate.ServiceListItem()
                            ...
                            .Paid(
                                fun e ->
                                    ActionDispatcher.RunAction ServiceFee.payFee (sessionID, p.ID) (Some updatePendingFeeList)
                            )
            ...
            .Submit(
                fun e ->
                        if JavaScript.JS.IsNaN e.Vars.Fee.Value then Feedback.giveFeedback true "Kérlek számot adj meg árnak!"
                            ...
                            ActionDispatcher.RunAction ServiceFee.submitPendingFee (sessionID,(int e.Vars.MemberSelectID.Value), (int e.Vars.Fee.Value)) (Some updatePendingFeeList)
            )
             .OnSelection(
                fun _ -> updateImageLists()
            )
            .UploaderList(
                Doc.BindSeqCached(
                    fun (u:Member) ->
                        SiteParts.ServiceTemplate.UploaderItem()
            ...
            .ImageList(
                Doc.BindSeqCached (
                    fun (fn,uploader,(date:System.DateTime)) ->
                        SiteParts.ServiceTemplate.ImageItem()
                            .ImageLink("/img/" + fn)
                            .UploadDate(string date.Year + "-" + sprintf "%02d" date.Month + "-" + sprintf "%02d" date.Day + " " + sprintf "%02d" date.Hour + ":" + sprintf "%02d" date.Minute)
                            .Delete(
                                fun e ->
                                    ActionDispatcher.RunAction ImageUpload.DeleteImage (sessionID,fn) (Some updateImageLists)
                            )
                            .Doc()
                ) currentImages
            )
