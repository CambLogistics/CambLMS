...
module TowPage =
    ...
    let updateAreaList () =
        async {
            let! list = CallsController.getAreaList ()
    ...
    let updateGarageList () =
        async {
            let! list = TowController.doGetGarageList ()
            garageList.Set list
        }
        |> Async.Start
    ...
    let RenderPage () =
        ...
        SiteParts
            .TowTemplate()
            .Price(
                selectedRoute.View.MapAsync(fun r ->
                    async {
                        if r.Source = -1 || r.Dest = -1 then
                            ...
                            let! price = TowController.doCalculatePrice r.Source r.Dest
                            let! isDouble = CallsController.clientGetDPStatus ()

                            if price > 0 then
                                JavaScript.JS.Document.GetElementById("Submit").RemoveAttribute("disabled")

                            return
                                (getAreaName r.Source
                                 + " - "
                                 + getGarageName r.Dest
                                 + ": "
                                 + string price
                                 + "$"
                                 + if isDouble then " (EMELT)" else "")
                    })
            )
            .LocationList(
                Doc.BindSeqCached
            ...
            .Submit(fun e ->
                async {
                    ...
                    ActionDispatcher.RunAction
                        TowController.submitCall
                        (sessionID, selectedRoute.Value.Source, selectedRoute.Value.Dest)
                        None
                }
                |> Async.Start)
            .Reset(fun e ->
